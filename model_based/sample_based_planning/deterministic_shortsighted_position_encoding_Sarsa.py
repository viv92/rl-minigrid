# Determinstic start, shortsighted agent, position encoding based table lookup model, sample based planning with sarsa
...
### function to sample a trajectory from learnt model - random starting state
def sample_trajectory(state_transition_model, reward_model, q_table, num_actions, eps):
    # trajectory conatiner - [state, action, reward, next state]
    trajectory = []
    trajectory_length = 0
    # random start state
    key = np.random.choice(list(state_transition_model.keys()))
    posenc, done = state_transition_model[key]
    # first action for sarsa
        ...
        if next_key not in reward_model: # pick an action for which experience is available
            ...
            for tmp_action in range(num_actions):
                ...
                if tmp_key in reward_model:
                    available_keys.append([tmp_key, tmp_action])
            random_index = random.randint(0, len(available_keys)-1)
            next_key, next_action = available_keys[random_index]
        # append to trajectory
        trajectory.append([posenc, action, reward, next_posenc, next_action])
        trajectory_length += 1
...
def run(env, num_episodes, planning_iterations, eps, lr, df):
    ...
    for i_episode in range(num_episodes):
        ...
        for k in range(planning_iterations):
            print('k:',k)
            # sample a trajectory from our model
            trajectory = sample_trajectory(state_transition_model, reward_model, q_table, num_actions, eps)
            # learn from sampled trajectory
...
planning_trajectory_maxsteps = 10000 # to avoid infinitely long trajectory during planning
eps = 1 # decayed
lr = 0.01
df = 0.99
