...
# Simulation policy updated after k planning iterations
...
def run(env, num_episodes, planning_iterations, eps, lr, df):
    ...
    for i_episode in range(num_episodes):
        ...
        while True:
            ...
            # simulation based planning - monte carlo tree search (using current simulation policy)
            k_trajectories = []
            ...
            for k in range(planning_iterations):
                ...
                k_trajectories.append(trajectory)
                k_trajectories_returns.append(trajectory_return)
            # update q-table for all states in the k trajectories - also updates the simulation policy
            for trajectory, trajectory_return in zip(k_trajectories, k_trajectories_returns):
...
plt.legend()
fig.savefig('./plots/deterministic_shortsighted_mc_tree_search.png')
