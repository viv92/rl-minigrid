        ...
        while not rollout_done:
            ...
            if posenc in tree_posencs:
                ...
                if len(available_keys) == 0: # experience unavailable for any action from this state in our model
                    ...
                    # pick action according to values from q_table (q_table used as prediction function, inspired from MuZero)
                    available_keys_q_values = np.zeros(len(available_keys))
                    for k, (tmp_key, tmp_action) in enumerate(available_keys):
                        if tmp_key in q_table:
                            available_keys_q_values[k] = q_table[tmp_key]
                    random_prob = random.uniform(0,1)
                    if random_prob < eps:
                        chosen_index = np.random.choice(len(available_keys))
                    else:
                        chosen_index = np.argmax(available_keys_q_values)
                    key, action = available_keys[chosen_index]
...
plt.legend()
fig.savefig('./plots/corrected_MCTS_treePolicy_qTable_as_predictionFunction_rolloutMaxsteps='+str(rollout_maxsteps)+'_planIter='+str(planning_iterations)+'.png')
