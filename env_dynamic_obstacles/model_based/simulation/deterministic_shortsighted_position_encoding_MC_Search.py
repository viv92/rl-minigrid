# Determinstic start, shortsighted agent, position encoding based table lookup model, simulation based planning with monte carlo search
...
### function to simulate a trajectory over learnt model - starting from the current (state,action) - fixed simulation policy (uniformly random)
def simulate_trajectory(current_key, current_posenc, current_action, state_transition_model, reward_model, num_actions, df):
    ...
    if current_key not in state_transition_model: # cannot simulate further - exit
        return trajectory, trajectory_return

    # take the action in the model
    reward = reward_model[current_key]
    trajectory_return += (df ** trajectory_length) * reward
    next_posenc, done = state_transition_model[current_key]
    # append to trajectory
    trajectory.append([current_posenc, current_action, reward])
    trajectory_length += 1
    ...
    while not done:
        ...
        if key not in reward_model: # pick an action for which experience is available
            ...
            if len(available_keys) == 0: # experience not availabe for any action - cannot simulate further
                return trajectory, trajectory_return
            random_index = random.randint(0, len(available_keys)-1) # select a random but valid action
...
def run(env, num_episodes, planning_iterations, eps, lr, df):
    ...
    for i_episode in range(num_episodes):
        ...
        while True:
            ...
            # simulation based planning - monte carlo search (fixed simulation policy - select action uniformly random)
            for tmp_action in np.arange(num_actions): # for each action - simulate k episodes
                tmp_key = posenc + [tmp_action]
                tmp_key = str(tmp_key) # current (state,action) encoding
                k_trajectories_returns = []
                for k in range(planning_iterations):
                    # simulate a trajectory from current (state,action) - note that simulation policy is fixed to be uniformly random
                    trajectory, trajectory_return = simulate_trajectory(tmp_key, posenc, tmp_action, state_transition_model, reward_model, num_actions, df)
                    k_trajectories_returns.append(trajectory_return)
                # update q value only for the current (state,action) - to the mean return of all simulated trajectories
                q_table[tmp_key] = np.mean(k_trajectories_returns)
...
plt.legend()
fig.savefig('./plots/deterministic_shortsighted_mc_search.png')
